...
#  It clusters the MERS Hyb data to create chimeric groups     #
################################################################
...
hybListSarsSwap = swapHybs(hybListMers, IDMers)
...
#save(longRange, file = "./longRangeclusteringMERS.RData")
...
#save(combinedPlotting, file = "./combinedClusteringMERS.RData")
...
for(j in 1:length(combinedPlotting)){
  ...
  #clusterPositions = read.table("../clustering/SARS-CoV-2_cluster_positions.txt", header = T)
  
  matList[[j]] = matrix(0,nrow = 30111, ncol = 30111)
  ...
  pdf(paste("../clustering/cluster_initialClustersMERS_S",j,".pdf"), height = 5, width = 5)
  heatmap3((log2((ss+1))), col=myCol, scale="none" ,Rowv = NA, Colv = NA)
  dev.off()
  
  
 # write.table(clusterPositionsList[[j]],paste( "../clustering/MERS_cluster_positions_S",j,".txt"), sep = "\t",row.names = F, quote = F)
...
for(z in 1:4){
  ...
  superclustersPlotting[[z]]  = printClustersFast("../clustering/",clustering, highest_clusters, left, right)
  ...
  #save.image("sarsCluseringShortAndLong_superClusters.RData")
  
  plottingListFull 
  cluster = mcols(plottingListFull)$cluster
  unique(plottingListFull$cluster)
  
  names(cluster)= names(plottingListFull) 
  #11186
  length(unique( mcols(plottingListFull)$cluster))
  
  
  #clusterPositions = clusterPositionsFull 
  nrow(clusterPositions)
  clusterPositions$superCluster = cluster[as.character((clusterPositions$id))]
  nrow(clusterPositions)
  unique(clusterPositions$superCluster)
  ...
  #clusterPositionsFull = clusterPositions
  clusterPositions = superclustersPoisitonList[[z]]
  
  
  mat = matrix(0,nrow = 30111, ncol = 30111)
  ...
  #pdf(paste("../clustering/superClustersMERS",z,".pdf"), height = 6, width = 6)
  #heatmap3((log2((ss+1))), col=myCol, scale="none" ,Rowv = NA, Colv = NA)
  #dev.off()
  #write.table(superclustersPoisitonList[[z]], paste("../clustering/superClusterspositionsMERS",z,".txt"), sep = "\t",row.names = F, quote = F)
...
#     superclustersPlotting ,file = "clustersAndSuperClustersMERS.rData")
...
for (i in 1:4){
  ...
  length(unique(  combinedPlottingUnlist$k))
  unique(  combinedPlottingUnlist$k)
  superclustersPoisitonList[[i]]
  library(stringr)
...
#save(allChimerasForSuperClustersPlotting, superclustersPoisitonList,file =  "superClustersBeforeTrimmingMERS.Rdata")
...
for(i in 1:4){
  ...
  for(cluster in unique(allChimerasForSuperClustersPlotting[[i]]$superCluster)){
    ...
    for(l in c("left","right")){
      ...
      #change the plotting positions of the cluster
      
      #chnge the actua cluster reads to make the ones shorter if nesseary 
...
allChimerasForSuperClustersPlotting
#save(allChimerasForSuperClustersPlottingTrimmed, file = "trimmedSuperCustersMERS.Rdata")
...
for(j in 1:4){
  ...
  clusterPositionsListTrimmed[[j]] = data.frame("id" = names(maxEnd)[seq(1,length(minStarts),2)],
                                                ...
                                                "re" = maxEnd[seq(2,length(maxEnd),2)],
                                                "size" = lengths[x,])
  sum(clusterPositionsListTrimmed[[j]]$size.x)
  row.names(lengths)

  
  matListTrimmed[[j]] = matrix(0,nrow = 30111, ncol = 30111)
  ...
  #pdf(paste("../clustering/superclusters_MERS_TRIMMED_clusters_TRIALisitthesameS",j,".pdf"), height = 5, width = 5)
  #heatmap3((log2((ss+1))), col=myCol, scale="none" ,Rowv = NA, Colv = NA)
  #dev.off()
 # write.table(clusterPositionsListTrimmed[[j]],paste( "../clustering/MERS_cluster_positions_TRIMMED_clusters_S",j,".txt"), sep = "\t",row.names = F, quote = F)
  
}

#save(clusterPositionsListTrimmed,allChimerasForSuperClustersPlottingTrimmed, file = "trimmedSuperCustersMERS.Rdata")
